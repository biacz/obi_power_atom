...
#define RELAY_PIN_ON 12
#define RELAY_PIN_OFF 5
...
void setup() {
  ...
  digitalWrite(RELAY_PIN_ON, HIGH);
...
void loop() {
  ...
  if (relayState != desiredRelayState) {
    digitalWrite(RELAY_PIN_OFF, desiredRelayState);
    digitalWrite(RELAY_PIN_ON, !desiredRelayState);
